...
in {
  options.myNixos.users = {
    ...
    users = { max = lib.mkEnableOption "Whether to create the test account."; };
  };
  ...
  config = {
    ...
    users.users = lib.mkMerge [
      ...
      (lib.mkIf cfg.max {
        max = {
          ...
          extraGroups = extraGroups;
          hashedPassword =
            config.mySecrets.getSecret "shared.users.users.max.hashedPassword";
          openssh.authorizedKeys.keys = authorizedKeys;
        };
      })
      {
        fine = {
          ...
          extraGroups = extraGroups;
          hashedPassword =
            config.mySecrets.getSecret "shared.users.users.fine.hashedPassword";
          openssh.authorizedKeys.keys = authorizedKeys;
        };
      }
    ];
    ...
    users.extraUsers = lib.mkMerge [
      ...
      (lib.mkIf cfg.max { max.extraGroups = [ "kvm" ]; })
    ];
  };
}
